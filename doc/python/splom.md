...
fig = go.Figure(go.Splom(
    dimensions=[
        dict(label="sepal length", values=df["sepal length"], axis=dict(matches=False)),
        dict(label="sepal width", values=df["sepal width"], axis=dict(matches=False)),
        dict(label="petal length", values=df["petal length"], axis=dict(matches=False)),
    ],
    marker=dict(size=4),
))

fig.update_layout(title=dict(text="Unlinked axes"), width=600, height=600)
fig.show()
```
